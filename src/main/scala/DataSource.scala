...
class DataSource(ep: EmptyParams)
  ...
  def selectEvents(sc: SparkContext): RDD[(String, PropertyMap)] = {
    val grades = PEventStore.aggregateProperties(
      ...
      required = Some(List("vendor","item","month","quantity"))
    )(sc)
    ...
    val events = grades.map {
      case (entityId, properties) =>
        val fields = Map(
          "vector" -> JArray(List(
             JDouble(properties.get[Double]("vendor")),JDouble(properties.get[Double]("item")),JDouble(properties.get[Double]("month"))
           )),
